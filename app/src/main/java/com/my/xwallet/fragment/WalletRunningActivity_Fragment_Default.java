            ...
            public List<TransactionInfo> runOnIo() {
                ...
                try {
                    if (transaction_type == null || transaction_type.equals(ActivityHelp.TRANSACTION_TYPE_ALL)) {
                        transactionInfos = AppDatabase.getInstance().transactionInfoDao().loadTransactionInfoByWalletId(wallet.getSymbol(), wallet.getId());
                    } else if (transaction_type.equals(ActivityHelp.TRANSACTION_TYPE_RECEIVE)) {
                        transactionInfos = AppDatabase.getInstance().transactionInfoDao().loadTransactionInfoByWalletIdAndDirection(wallet.getSymbol(), wallet.getId(), 0);
                    } else if (transaction_type.equals(ActivityHelp.TRANSACTION_TYPE_SEND)) {
                        transactionInfos = AppDatabase.getInstance().transactionInfoDao().loadTransactionInfoByWalletIdAndDirection(wallet.getSymbol(), wallet.getId(), 1);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
